...
//	----------�p�G���O�b�����r�A�N�B�z�W�s��--------------
		int l;	char* url;
		if ((url = HyperLinkHitTest(point, l)))	//�p�G�ƹ��I���W�s��
		{
...
//	�W�s�����
	...
	*/
	...
	if (sel.GetLength() > 0)
	{
		...
		search_menuiteminfo.hSubMenu = SearchPluginCollection.CreateSearchMenuFromConfig(sel);
		CString web_search;
		...
		if (AppConfig.enable_translation && sel.GetLength() <= SearchPluginCollection.MaxTranLength)
		{
	...
	if (telnet)
	{
		...
		if ((link = HyperLinkHitTest(pt, len)))	//�p�G������O�W�s��
		{
...
BOOL CTermView::Connect(CAddress address, CString name, LPCTSTR cfg_path)
{
	...
	CConn* ncon = NewConn(address, name, cfg_path);	//���ͤF�s���s�u�e���A�����Ҧ��]�w
	if (!ncon)
		return FALSE;
...
#if defined	_COMBO_
	if (!new_telnet->is_web)	//�p�G�O�s�uBBS,�s�}BBS�e��
	{
#endif
		new_telnet->cfg_path = cfg_path;

		//���s��socket���J�]�w��
		if (!new_telnet->site_settings.Load(new_telnet->cfg_path))	//�p�G���J�]�w�o�Ϳ��~
		{
			//�o�ܦ��i��|�b�K�X��J���~���ɭԵo��!
			delete new_telnet;
			return NULL;
...
char* CTermView::HyperLinkHitTest(CPoint client_point, int& len)	//�ΨӴ��յe���W�Y�I�O�_���W�s��
...
void CTermView::OnCurConSettings()
{
	...
	if (!telnet->cfg_path.IsEmpty())	//���ո��J�ӧO�]�w
	...
	if (dlg.DoModal() == IDOK)
	{
		...
		if (strncmp(telnet->site_settings.key_map_name, tmpset.key_map_name, 10))	//�p�G��L��M���
		{
...
BOOL CTermView::ExtTextOut(CDC& dc, int x, int y, UINT nOptions, LPCRECT lpRect, LPCTSTR lpszString, UINT nCount)
{
	...
	if (cp_id != 950)
	{
		...
		// in����dbcs, in[n]=out[n/2+1]; in����acsii, in[n]=out[n+1]
...
void CTermView::OnSearchPlugin(UINT id)
{
	...
	CString searchUrl = SearchPluginCollection.GetSearchUrlFromConfig(id, GetSelText());
	if (!searchUrl.IsEmpty())
	{
		AppConfig.hyper_links.OpenURL(searchUrl);
	}
	else
	{
		// Fallback to original method if config method fails
