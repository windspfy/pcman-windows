...
BEGIN_MESSAGE_MAP(CMainFrame, CFrameWnd)
	...
	ON_COMMAND_RANGE(ID_FIRST_HOTSTR, ID_LAST_HOTSTR, OnFrequentlyUsedStr)	//����e�X�r��
	ON_COMMAND_RANGE(ID_FIRST_BBS_FAVORITE, ID_LAST_WEB_FAVORITE, OnFavorite)	//�ڪ��̷R
	ON_COMMAND_RANGE(ID_SWITCHCON1, ID_SWITCHCON10, OnHotkeySwitch)	//��������
...
//	Create Main Toolbar
	...
	//�ϥ� bmBitsPixel �ӷ��@ ILC_COLOR* �� flags ��
#ifdef _COMBO_
...
//	�D�`�޲��A�N�~�o�{�A�p�G�@�}�l�إ�OwnerDraw�AWin XP�UHotTrack�N�|����
//	���O�p�G���إߤ@�몺HotTrack Tab�A�A�令OwnerDraw�A�N�i�H�O�dHotTrack :)
#if defined(_COMBO_)
...
void CMainFrame::UpdateStatus()
{
	...
	if (telnet)
	{
		if (! telnet->is_ansi_editor)
		{
			...
			min = min % 60;
//			text.Format("\t�s�u�ɶ��G%d �p�� %d �� %d ��\t\t��}�G%s",hr,min,sec,(LPCTSTR)telnet->address);
...
//			text.Format( "ANSI�m��s��\t�s��Ҧ��G%s\t�ɮסG%s", LPCTSTR(mode), LPCTSTR(file) );
...
void CMainFrame::RecalcLayout(BOOL bNotify)
{
	...
	if (AppConfig.kktab)	//�p�G�ϥΩMKKman�ۦP���s�u����
	{
...
void CMainFrame::OnWebSearch()
{
	...
	else
	{
		...
		*/
		OnNewConnectionAds(searchurl);	// �����|�I�s view.AddToTypedHistory(address);
	}
...
void CMainFrame::RestoreWindow()
{
	...
	if (IsIconic())	// Restore minimized window �q Delphi VCL�ǨӪ�!
...
{
	if (AppConfig.is_full_scr)	//�p�G�w�g�O���ù�
	{
...
void CMainFrame::OnPasteTinyUrl()
{
	...
	CString text;
	if (!CClipboard::GetText(text)) //�Ĥ@���q�ŶKï���r�� (ansi�r��)
		return;
	url += text;
...
void CMainFrame::OnPaste()
{
	...
	{
		//�åB�p�G���]�t��m�X�A�A���s�q�ŶKï���@���r�� (���Unicode�r��)
...
void CMainFrame::OnBBSFont()
{
	...
	if (dlg.DoModal() == IDOK)
	{
		...
		else
		{
			...
			CSize& sz = dc.GetTextExtent(LoadString(IDS_DOUBLE_SPACE_CHAR) , 2);	// ���Ϊť�font��size
			dc.SelectObject(&old);
